...
export function createProviderRoutes(providerService, tmdbService) {
    ...
    router.get('/movie/:tmdbId', async (req, res) => {
        try {
            ...
            // Scrape streams
            const streams = await providerService.scrapeMovie(movieDetails, providerList, parseInt(timeout, 10));
    ...
    router.get('/tv/:tmdbId/:season/:episode', async (req, res) => {
        try {
            ...
            const showDetails = await tmdbService.getShowDetails(tmdbId, parseInt(season, 10), parseInt(episode, 10));
            // Parse requested providers
            let providerList;
            if (typeof requestedProviders === 'string') {
                providerList = requestedProviders.split(',');
            ...
            // Scrape streams
            const streams = await providerService.scrapeTvShow(showDetails, providerList, parseInt(timeout, 10));
            // Get available providers for response
            const availableProviders = providerService.getAvailableProviders();
            res.json({
                success: true,
    ...
    router.get('/tmdb/search', async (req, res) => {
        try {
            ...
            const results = await tmdbService.searchContent(query, type, parseInt(page, 10));
            res.json({
                success: true,
                results,
            });
